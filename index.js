...
Ketik ${prefix}verify untuk memulai menggunakan bot.`
	...
	Lxa.on('chat-update', async (mek) => {
      ...
      const is = budy.slice(0).trim().split(/ +/).shift().toLowerCase()
			mess = {
  wait: '*Ok amore 1 mint mientras tanto, tómate un ☕ con 🍪...*',
  success: '*Funciona xd*',
  Public: '*Lo siento, Estoy privado para que solo lo use mi dueño. 😏 Intenta más tarde*',
  ferr: 'Falta algo o escribiste un comando incorrecto ;v',
  limitend: 'Ops amigo(a), su límite ha terminado, comuníquese con mi dueño para restablecer su límite 😔',
  error: {
  stick: '*Lo siento, tengo erros, prueba este comando más tarde 😔🔫*',
  Iv: '*Link Error*'
  },
  only: {
    group: 'Este comando es solo para *grupos*',
    ownerG: 'Este comando es solo para el *dueño del grupo*',
    ownerB: 'Este comando es solo para mi *dueño*',
    admin: 'Este comando es solo para *admins* macaco ;v',
    Badmin: '*☒* Jadikan Lexa *ADMIN !*',
    daftarB: `Hola, escribe *${prefix}verify* para comenzar a usar el bot`
  }
}
...
tm = `La verificación está completa, escriba *${prefix}menu* para mostrar la lista del menu
`
reply(tm)
break
